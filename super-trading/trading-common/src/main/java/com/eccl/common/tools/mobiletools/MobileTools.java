package com.eccl.common.tools.mobiletools;
...
import com.eccl.common.util.iputil.IpUtil;
...
public class MobileTools {
	...
	private static String IP_FILE = MobileTools.class.getResource("QQWry.dat").getPath();
	...
	// 单一模式实例
	private static MobileTools instance = new MobileTools();
	// 起始地区的开始和结束的绝对偏移
	...
	public MobileTools() {
		...
		try {
			...
			System.out.println(MobileTools.class.getResource("QQWry.dat").getPath());
	...
	private String readString(long offset) {
		try {
			...
			for (i = 0, buf[i] = ipFile.readByte(); buf[i] != 0; buf[++i] = ipFile.readByte())
				;
			if (i != 0)
				return StringUtil.byteStrToStr(buf, 0, i, "GBK");
		} catch (IOException e) {
			System.out.println(e.getMessage());
		}
		return "";
	...
	public static void main(String[] args) {
		MobileTools ipseeker = new MobileTools();
		String ip = "112.126.71.45";
		// String result = ipseeker.getAddress("124.237.112.74");
	}
}